...
void Crawler::networking() {
  ...
  while (!_links_queue.empty()) {
    ...
    auto url = _links_queue.front().url;
    auto depth = _links_queue.front().depth;
    _links_queue.pop();
    cs.unlock();
    make_link_vector(url, depth);
  }
}
...
void Crawler::parsing() {
  ...
  while (!_pars_queue.empty()) {
    cs.lock();
    auto from = _pars_queue.front();
    _pars_queue.pop();
    cs.unlock();
    GumboOutput *output = gumbo_parse(from.body.c_str());
    search_for_img(output->root, from.url);
  }
}
...
std::string Crawler::https_downloader(std::string port, std::string url) {
  ...
  boost::beast::http::request<boost::beast::http::string_body>
      request{boost::beast::http::verb::get, target, version};
